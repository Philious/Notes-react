...
import { NoteProps, NoteResponse } from '@/types/types';  // Adjust import based on your project
...
export const updateNote = createAsyncThunk<NoteResponse, Partial<NoteProps> & { id: string }>(
  'notes/updateNote',
  async (note, thunkURL) => {
    try {
      const response = await backend.put<NoteResponse>(`Notes/${note.id}`, note);
      return response.data;
    } catch (error) {
